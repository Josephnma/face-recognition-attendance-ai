...
import pyttsx3
from register_student import register_student  # Import the register_student function
...
# Ensure the student image directory exists
...
@app.route('/')
def index():
    return render_template('index2.html')  # Assuming the template name
...
def register():
    ...
    image_filename = f"{student_name.replace(' ', '_')}.jpg"
    image_path = os.path.join(STUDENT_IMAGE_FOLDER, image_filename)

    # Save the image to the defined path
    with open(image_path, 'wb') as f:
        f.write(image_data)

    # Register the student in the CSV file
    register_student(student_name, image_filename)
